...
export function Committed({ data }: { data: CommittedEventFragmentFragment }) {
  return (
    <div className="flex gap-2">
      ...
      <div className="flex gap-2 items-center mt-1 flex-1">
        <Avatar src={data.commitedCommit.author!.avatarUrl} />
        <span className="flex-1 line-clamp-2 text-muted-foreground"><Link to={data.commitedCommit.commitUrl} target="_blank" className="hover:underline text-muted-foreground">{data.commitedCommit.message}</Link></span>
