...
type PullRequestsListProps = {
  owner: string
  repo: string
  search?: { author?: string, state?: string, sort?: string, perPage?: number }
}

export function RepoPullRequestsList({ owner, repo, search }: PullRequestsListProps) {
  const ref = useRef<HTMLInputElement>(null)
  const { data } = usePRsQuery(owner, repo, search)
